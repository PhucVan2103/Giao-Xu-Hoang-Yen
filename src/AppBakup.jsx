...
// --- ICON FACEBOOK TỰ VẼ (TRÁNH LỖI THƯ VIỆN) ---
...
const navLinks = [
  ...
  { name: 'Giáo Lý', id: 'Catechism' },
  { name: 'Hội Đoàn', id: 'Groups' },
...
const initialNewsData = [
  { 
    id: 1, 
    title: 'Đại lễ Kính Các Thánh Tử Đạo Việt Nam', 
    date: '14/11/2023', 
    category: 'Sự kiện',
    isFeatured: true,
  ...
  { 
    id: 2, 
    title: 'Thông báo Giáo lý niên khóa mới', 
    date: '10/11/2023', 
    category: 'Giáo lý',
    isFeatured: true,
  ...
  { 
    id: 3, 
    title: 'Bản tin Bác ái: Chuyến đi thăm mái ấm tình thương', 
    date: '05/11/2023', 
    category: 'Bác ái',
    isFeatured: false,
  ...
  { 
    id: 4, 
    title: 'Thư ngỏ: Đóng góp xây dựng nhà giáo lý', 
    date: '01/11/2023', 
    category: 'Thông báo',
    isFeatured: false,
  ...
  { 
    id: 5, 
    title: 'Thánh lễ ban Bí tích Thêm Sức năm 2023', 
    date: '25/10/2023', 
    category: 'Tin Tức',
    isFeatured: false,
  ...
  { 
    id: 6, 
    title: 'Lịch tĩnh tâm Mùa Vọng 2023 dành cho các giới', 
    date: '20/10/2023', 
    category: 'Sự kiện',
    isFeatured: false,
...
const initialPilgrimageData = [
  {
    id: 1,
    title: 'Hành Hương Đức Mẹ La Vang - Thánh Địa Trà Kiệu',
    date: '15/06/2024',
    duration: '3 Ngày 2 Đêm',
    status: 'Đang mở đăng ký',
    desc: 'Hành trình thiêng liêng về với Đức Mẹ, kết hợp tham quan các di tích lịch sử Công giáo miền Trung. Kính mời cộng đoàn đăng ký.',
    image: 'https://images.unsplash.com/photo-1548625361-903df390453d?q=80&w=800&auto=format&fit=crop',
  ...
  {
    id: 2,
    title: 'Chương Trình Đón Tiếp Đoàn Hành Hương Giáo Phận',
    date: '24/11/2024',
    duration: '1 Ngày (Lễ Các Thánh TĐVN)',
    status: 'Sắp diễn ra',
  ...
  {
    id: 3,
    title: 'Hành Hương Về Nguồn: Đền Thánh Martinô',
    date: '05/04/2024',
    duration: '1 Ngày',
    status: 'Đã kết thúc',
    desc: 'Chuyến đi tĩnh tâm ngắn ngày dành riêng cho Giới Trẻ giáo xứ để tìm hiểu về tấm gương khiêm nhường của Thánh Martinô.',
    image: 'https://images.unsplash.com/photo-1438232992991-995b7058bbb3?q=80&w=800&auto=format&fit=crop',
    content: '<h3>Tóm tắt chuyến đi</h3><p>Đoàn đã có một ngày thật ý nghĩa với Thánh lễ tạ ơn và các hoạt động teambuilding gắn kết tinh thần đồng đội.</p>'
  },
  {
    id: 4,
    title: 'Hành Hương Đức Mẹ Tà Pao',
    date: '13/10/2024',
    duration: '2 Ngày 1 Đêm',
    status: 'Đang mở đăng ký',
    desc: 'Cùng nhau về bên Mẹ Tà Pao dâng chuỗi Mân Côi và tham dự đêm canh thức rước nến lung linh giữa núi rừng thiêng liêng.',
    image: 'https://images.unsplash.com/photo-1510936111840-65e151ad71bb?q=80&w=800&auto=format&fit=crop',
    content: '<h3>Chương trình dự kiến</h3><p>Chi phí: 850.000 VNĐ / người (Bao gồm xe di chuyển, ăn uống và bảo hiểm). Hạn chót đăng ký: 01/10/2024.</p>'
  },
  {
    id: 5,
    title: 'Hành Hương Các Nhà Thờ Cổ Nam Định',
    date: '12/12/2023',
    duration: '2 Ngày',
    status: 'Đã kết thúc',
    desc: 'Chiêm ngưỡng vẻ đẹp kiến trúc tráng lệ của các vương cung thánh đường và nhà thờ cổ kính tại Nam Định.',
    image: 'https://images.unsplash.com/photo-1469571486292-0ba58a3f068b?q=80&w=800&auto=format&fit=crop',
    content: '<h3>Hồi ký</h3><p>Chuyến đi đã để lại nhiều ấn tượng sâu sắc về đức tin kiên trung của các bậc tiền nhân tại vùng đất Nam Định.</p>'
  }
];
...
const getDaysArray = (year, month) => {
  ...
  const startingDayIndex = firstDay === 0 ? 6 : firstDay - 1; // Mon = 0
  const days = [];
...
// --- HỆ THỐNG CLASS CSS DÙNG CHUNG CHO BÀI VIẾT (CHUẨN BÁO CHÍ) ---
...
const Logo = ({ sizeClass = "w-12 h-12", isSolid }) => (
...
const RichTextEditor = ({ value, onChange, minHeight = "150px" }) => {
  ...
  const handleEditorClick = (e) => {
    if (selectedImg && selectedImg !== e.target) selectedImg.style.outline = 'none';
    if (e.target.tagName === 'IMG') {
      e.target.style.outline = '3px solid #ec4899';
  ...
  const handleAlign = (alignType) => {
    if (selectedImg) {
      if (alignType === 'left') {
        selectedImg.style.display = 'inline-block'; selectedImg.style.float = 'left'; selectedImg.style.margin = '0.5rem 1.5rem 1rem 0';
      } else if (alignType === 'right') {
        selectedImg.style.display = 'inline-block'; selectedImg.style.float = 'right'; selectedImg.style.margin = '0.5rem 0 1rem 1.5rem';
      } else {
        selectedImg.style.display = 'block'; selectedImg.style.float = 'none'; selectedImg.style.margin = '1.5rem auto';
      }
  ...
  return (
    <div className="border border-pink-200 rounded-sm flex flex-col bg-white overflow-hidden shadow-sm">
      <div className="bg-pink-50 p-2 flex flex-wrap gap-1 border-b border-pink-200 items-center">
        <select onMouseDown={e => e.preventDefault()} onChange={(e) => execCmd('formatBlock', e.target.value)} className="text-xs p-1 border border-pink-200 rounded bg-white text-stone-700 outline-none cursor-pointer font-bold" defaultValue="P">
          <option value="P">Đoạn văn</option><option value="H3">Tiêu đề Lớn</option><option value="H4">Tiêu đề Nhỏ</option><option value="BLOCKQUOTE">Trích dẫn</option>
        </select>
        ...
        <div className="w-px h-4 bg-pink-200 mx-1"></div>
        <button type="button" onMouseDown={e => e.preventDefault()} onClick={() => handleAlign('left')} className="p-1.5 hover:bg-pink-200 rounded text-stone-700"><AlignLeft size={14}/></button>
        ...
        <div className="flex items-center ml-2 space-x-2 bg-white px-2 py-0.5 rounded border border-pink-200">
           <input type="color" onMouseDown={e => e.preventDefault()} onChange={(e) => execCmd('foreColor', e.target.value)} className="w-4 h-4 p-0 border-0 rounded cursor-pointer bg-transparent" />
        </div>
      </div>
      <div
        ref={editorRef} contentEditable onInput={handleInput} onPaste={handlePaste} onClick={handleEditorClick} onBlur={handleBlur}
        className={`p-4 focus:outline-none custom-scrollbar overflow-y-auto ${editorContentClasses}`} style={{ minHeight }}
      />
    </div>
  );
};
...
const App = () => {
  ...
  const [tempQuote, setTempQuote] = useState({ text: '', ref: '' });
  ...
  const [pastoralData, setPastoralData] = useState({
    ...
    content: `<h4>01. Đào Tạo Đức Tin Giới Trẻ</h4><p>Chú trọng sâu sát vào việc giáo dục nhân bản và giáo lý cho thiếu nhi, thanh giới trẻ. Đây là nòng cốt để xây dựng thế hệ tương lai vững vàng trong xã hội nhiều biến động.</p><h4>02. Thực Thi Bác Ái Xã Hội</h4><p>Mở rộng vòng tay yêu thương đến những người có hoàn cảnh khó khăn, ốm đau. Giáo xứ không chỉ là nơi cầu nguyện mà còn là điểm tựa tinh thần và vật chất cho người nghèo.</p><h4>03. Đồng Hành Cùng Gia Đình</h4><p>Đồng hành và nâng đỡ các gia đình trẻ, biến mỗi gia đình thành một "Hội Thánh tại gia" đúng nghĩa, nơi ngập tràn tiếng cười và sự cảm thông.</p>`
  });
  const [editingPastoral, setEditingPastoral] = useState(false);
  const [tempPastoral, setTempPastoral] = useState({ title: "", content: "" });
  ...
  const [tempLiturgyEvent, setTempLiturgyEvent] = useState({ title: '', colorType: 'white', desc: '' });
  ...
  const [tempConfession, setTempConfession] = useState({ title: '', desc: '' });
  ...
  const [tempAdoration, setTempAdoration] = useState({ title: '', desc: '' });
  ...
  const handleLogin = () => {
    if (password === 'admin') { 
      setIsAdmin(true); setShowLoginModal(false); setPassword(''); setLoginError('');
    } else {
  ...
  const renderContent = () => {
    switch (activeTab) {
          ...
          <div className="py-20 md:py-24 bg-[#fffcfd] min-h-screen text-stone-900 animate-in fade-in duration-500">
            <div className="max-w-6xl mx-auto px-4 lg:px-6">
              ...
              <div className="text-center mb-12 relative">
                ...
                {isAdmin && (
                  <button onClick={() => { setTempNews({ id: Date.now(), title: '', date: getTodayFormatted(), category: 'Tin Tức', desc: '', image: '', content: '', isFeatured: false }); setEditingNews('new'); }} className="absolute right-0 top-6 px-4 py-2 bg-pink-600 text-white text-[11px] font-bold rounded shadow-md transition-all active:scale-95 hidden md:flex items-center gap-1">
                    + Thêm Bản Tin
                  </button>
                )}
              </div>
              {isAdmin && (
                <button onClick={() => { setTempNews({ id: Date.now(), title: '', date: getTodayFormatted(), category: 'Tin Tức', desc: '', image: '', content: '', isFeatured: false }); setEditingNews('new'); }} className="mb-8 w-full py-3 bg-pink-600 text-white text-xs font-bold rounded shadow-md transition active:scale-95 flex md:hidden items-center justify-center gap-2">
                  + Thêm Bản Tin
                </button>
              )}
      ...
      case 'Liturgy':
        ...
        const eventOfSelectedDate = liturgyEvents.find(e => e.date === selectedDateStr);
        const eventColorObj = eventOfSelectedDate ? litColors[eventOfSelectedDate.colorType] : litColors.white;
          ...
          <div className="py-24 bg-[#fffcfd] min-h-screen text-stone-900 animate-in fade-in duration-500">
            <div className="max-w-6xl mx-auto px-4 lg:px-6">
              ...
              <div className="text-center mb-12">
                <h2 className="text-3xl md:text-4xl font-serif font-bold text-pink-950 mb-4 uppercase tracking-widest leading-tight">Lịch Phụng Vụ & Bí Tích</h2>
                <div className="flex items-center justify-center space-x-3 mb-4">
                  <div className="h-[1px] w-12 bg-pink-200"></div><div className="text-pink-300 text-sm">❦</div><div className="h-[1px] w-12 bg-pink-200"></div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
                <div className="bg-white p-6 shadow-lg shadow-pink-100 border-t-4 border-pink-500 rounded-xl relative group col-span-1 md:col-span-2 lg:col-span-1">
                  {isAdmin && <button onClick={() => { setTempMass([...massSchedules]); setEditingMass(true); }} className="absolute -top-3 right-3 p-1.5 bg-pink-600 text-white rounded-full shadow-md transition active:scale-90"><Edit3 size={14} /></button>}
                  <h3 className="text-lg font-bold mb-5 uppercase flex items-center tracking-tight text-pink-700"><Clock className="mr-2 text-pink-500" size={18}/> Giờ Lễ Chung</h3>
                  <div className="space-y-3">
                    {massSchedules.map((item, idx) => (
                      <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-stone-50 rounded-lg">
                        <p className="font-bold text-pink-800 uppercase text-[10px] tracking-wider mb-1.5 sm:mb-0">{item.day}</p>
                        <div className="flex gap-1.5 flex-wrap">{item.times.map((t, i) => <span key={i} className="text-[10px] font-bold bg-white text-pink-900 border border-pink-200 px-1.5 py-0.5 rounded shadow-sm">{t}</span>)}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-white p-6 shadow-lg shadow-purple-100 border-t-4 border-purple-500 rounded-xl relative group">
                  {isAdmin && <button onClick={() => { setTempConfession(confessionData); setEditingConfession(true); }} className="absolute -top-3 right-3 p-1.5 bg-purple-600 text-white rounded-full shadow-md transition active:scale-90"><Edit3 size={14} /></button>}
                  <h3 className="text-lg font-bold text-purple-700 mb-3 flex items-center tracking-tight"><Heart className="mr-2 text-purple-500" size={18}/> {confessionData.title}</h3>
                ...
                <div className="bg-white p-6 shadow-lg shadow-amber-100 border-t-4 border-amber-500 rounded-xl relative group">
                  {isAdmin && <button onClick={() => { setTempAdoration(adorationData); setEditingAdoration(true); }} className="absolute -top-3 right-3 p-1.5 bg-amber-500 text-white rounded-full shadow-md transition active:scale-90"><Edit3 size={14} /></button>}
                  <h3 className="text-lg font-bold text-amber-700 mb-3 flex items-center tracking-tight"><BookOpen className="mr-2 text-amber-500" size={18}/> {adorationData.title}</h3>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg border border-pink-100 p-6">
                  <div className="flex justify-between items-center mb-6">
                    <button onClick={prevMonth} className="p-2 hover:bg-pink-50 text-pink-700 rounded-full transition"><ChevronLeft size={20}/></button>
                    <h3 className="text-lg md:text-xl font-bold font-serif text-pink-950 uppercase tracking-widest">
                      Tháng {calendarDate.getMonth() + 1} / {calendarDate.getFullYear()}
                    </h3>
                    <button onClick={nextMonth} className="p-2 hover:bg-pink-50 text-pink-700 rounded-full transition"><ChevronRight size={20}/></button>
                  </div>
                  
                  <div className="grid grid-cols-7 gap-2 md:gap-3 mb-2">
                    {['T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'CN'].map((d, i) => (
                      <div key={d} className={`text-center font-bold text-xs md:text-sm uppercase tracking-widest pb-2 ${i === 6 ? 'text-red-600' : 'text-stone-400'}`}>{d}</div>
                    ))}
                    
                    {getDaysArray(calendarDate.getFullYear(), calendarDate.getMonth()).map((d, idx) => {
                      if (d === null) return <div key={`empty-${idx}`} className="bg-transparent rounded-lg"></div>;
                      
                      const cellDateStr = formatDateString(new Date(calendarDate.getFullYear(), calendarDate.getMonth(), d));
                      const isSelected = cellDateStr === selectedDateStr;
                      const event = liturgyEvents.find(e => e.date === cellDateStr);
                      const isSunday = (idx + 1) % 7 === 0;

                      return (
                        <div 
                          key={idx} 
                          onClick={() => setSelectedDate(new Date(calendarDate.getFullYear(), calendarDate.getMonth(), d))}
                          className={`min-h-[70px] md:min-h-[90px] p-1 md:p-2 rounded-xl border transition-all cursor-pointer relative flex flex-col ${isSelected ? 'border-pink-500 bg-pink-50 shadow-md ring-2 ring-pink-200' : 'border-stone-100 bg-white hover:border-pink-300'}`}
                        >
                          <span className={`text-sm md:text-base font-bold text-center ${isSunday ? 'text-red-600' : 'text-stone-700'}`}>{d}</span>
                          {event && (
                            <div className={`mt-auto text-[9px] md:text-[10px] p-1 rounded font-serif line-clamp-2 leading-tight border ${litColors[event.colorType].bg} ${litColors[event.colorType].text} ${litColors[event.colorType].border}`}>
                              {event.title}
                            </div>
                          )}
                        </div>
                ...
                <div className="lg:col-span-1">
                  <div className={`rounded-2xl shadow-lg border p-6 md:p-8 h-full flex flex-col transition-colors duration-500 ${eventColorObj.bg} ${eventColorObj.border}`}>
                    <div className="flex justify-between items-start mb-6">
                      <div className="text-[11px] font-bold uppercase tracking-widest text-stone-500 flex items-center">
                        <Calendar size={14} className="mr-2" />
                        Ngày {String(selectedDate.getDate()).padStart(2, '0')}/{String(selectedDate.getMonth() + 1).padStart(2, '0')}/{selectedDate.getFullYear()}
                      </div>
                      {isAdmin && (
                        <button onClick={() => {
                          if (eventOfSelectedDate) {
                            setTempLiturgyEvent(eventOfSelectedDate);
                          } else {
                            setTempLiturgyEvent({ date: selectedDateStr, title: '', colorType: 'white', desc: '' });
                          }
                          setEditingLiturgyEvent(true);
                        }} className="p-1.5 bg-white/50 text-stone-700 hover:text-pink-700 rounded-full shadow-sm transition"><Edit3 size={14}/></button>
                      )}
                    </div>

                    {eventOfSelectedDate ? (
                      <>
                        <h4 className={`text-2xl font-serif font-bold mb-4 leading-tight ${eventColorObj.text}`}>{eventOfSelectedDate.title}</h4>
                        <div className={`inline-block px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest mb-6 border bg-white/50 ${eventColorObj.text} ${eventColorObj.border}`}>
                          Màu áo: {eventColorObj.name}
                        </div>
                        <p className="font-serif text-sm leading-relaxed text-stone-700 flex-1">{eventOfSelectedDate.desc}</p>
                      </>
                    ) : (
                      <div className="flex-1 flex flex-col items-center justify-center opacity-50 text-stone-500">
                        <BookOpen size={40} className="mb-4" />
                        <p className="font-serif text-sm text-center">Không có sự kiện phụng vụ đặc biệt.<br/>Cử hành theo lịch ngày thường.</p>
                      </div>
                    )}
                  </div>
                </div>
        ...
        return (
          <>
            <section className="relative h-screen flex items-center justify-center text-white overflow-hidden">
              ...
              <div className="absolute inset-0 bg-cover bg-center transition-transform duration-[30000ms] scale-105" style={{ backgroundImage: 'url("https://travelplusvn.com/public/uploads/images/Bai_Viet/11_mon_do_khong_the_thieu/Nha-tho-Duc-Ba-1.jpg")' }}></div>
              <div className="relative z-20 w-full max-w-6xl px-4 flex flex-col items-center animate-in fade-in slide-in-from-bottom-10 duration-1000 text-center">
                <h2 className="text-pink-200 text-xs md:text-sm uppercase tracking-[0.5em] mb-4 font-bold drop-shadow-md">Chào mừng quý vị đến với</h2>
                <h1 className="text-4xl sm:text-5xl md:text-6xl font-serif font-bold tracking-tight text-center drop-shadow-2xl leading-tight w-full uppercase">GIÁO XỨ HOÀNG YÊN</h1>
                <p className="text-base md:text-lg font-serif font-light text-pink-50 mb-10 italic tracking-wider opacity-90">Đền Thánh Nữ Vương Các Thánh Tử Đạo Việt Nam</p>
                <div className="flex flex-col sm:flex-row gap-4">
                  <button onClick={() => setActiveTab('Liturgy')} className="px-8 py-3 bg-pink-700 hover:bg-pink-800 text-white rounded font-bold text-[11px] shadow-lg transition-all active:scale-95 tracking-widest uppercase">Lịch Phụng Vụ</button>
                  <button onClick={() => setActiveTab('Pilgrimage')} className="px-8 py-3 bg-transparent hover:bg-white/10 text-white border border-white/50 rounded font-bold text-[11px] transition-all active:scale-95 tracking-widest uppercase">Hành Hương</button>
                </div>
              </div>
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 animate-bounce text-pink-200/50"><ChevronDown size={32} /></div>
            </section>

            <section className="py-16 md:py-20 bg-[#fff8f9] relative overflow-hidden text-stone-900 border-b border-pink-50">
              <div className="max-w-6xl mx-auto px-4 relative z-10 grid grid-cols-1 md:grid-cols-3 gap-10">
                  <div className="md:col-span-1 relative group">
                    {isAdmin && <button onClick={() => { setTempQuote(quote); setEditingQuote(true); }} className="absolute top-3 right-3 p-1.5 bg-pink-600 text-white rounded-full transition active:scale-90 shadow-md"><Edit3 size={14} /></button>}
                    <div className={`bg-white p-8 shadow-sm border ${isAdmin ? 'border-pink-300' : 'border-pink-100'} h-full text-center rounded-xl transition-all hover:shadow-md`}>
                      <Quote className="text-pink-100 w-10 h-10 mb-4 mx-auto" />
                      <h3 className="text-lg font-bold text-pink-900 mb-4 uppercase border-b border-pink-100 pb-2 tracking-tight">Lời Chúa Hôm Nay</h3>
                      <div className="text-stone-700 font-serif italic text-base leading-relaxed mb-6 [&_img]:max-w-full [&_img]:h-auto [&_img]:rounded-md [&_img]:my-3" dangerouslySetInnerHTML={{ __html: quote.text }} />
                      <p className="text-pink-800 font-bold text-xs tracking-widest uppercase">{quote.ref}</p>
                    </div>
                  </div>
                  <div className="md:col-span-1 text-white">
                    <div className="bg-pink-950 p-8 shadow-xl h-full border-t-4 border-pink-400 rounded-xl relative group">
                      {isAdmin && <button onClick={() => { setTempMass([...massSchedules]); setEditingMass(true); }} className="absolute top-3 right-3 p-1.5 bg-pink-600 text-white rounded-full transition active:scale-90"><Edit3 size={14} /></button>}
                      <div className="flex flex-col items-center mb-6"><Clock className="text-pink-300 mb-3" size={28} /><h3 className="text-xl font-bold uppercase text-pink-100 tracking-tight">Giờ Thánh Lễ</h3></div>
                      <div className="space-y-6">{massSchedules.map((item, idx) => (<div key={idx} className="border-b border-pink-900/50 pb-3 last:border-0"><p className="font-bold text-pink-300 uppercase text-[11px] mb-2 tracking-wider">{item.day}</p><div className="flex flex-wrap gap-2">{item.times.map((t, i) => <span key={i} className="text-[10px] font-bold opacity-90 bg-pink-900/50 px-2 py-1 rounded border border-pink-800">{t}</span>)}</div></div>))}</div>
                    </div>
                  </div>
                  <div className="md:col-span-1 bg-white p-8 border border-pink-100 rounded-xl shadow-sm flex flex-col">
                    <h3 className="text-lg font-bold text-pink-900 mb-6 uppercase border-b border-pink-50 pb-2 tracking-tight">Thông Báo Mới</h3>
                    <div className="space-y-4 cursor-pointer flex-1" onClick={() => { if(newsItems[0]) { setLastTab('Home'); setSelectedNews(newsItems[0]); setActiveTab('NewsDetail'); window.scrollTo(0,0); } }}>
                      <p className="text-[9px] font-bold text-pink-600 uppercase tracking-widest">Tin nổi bật</p>
                      <p className="font-serif font-bold leading-snug hover:text-pink-700 transition text-base line-clamp-3">{newsItems[0]?.title || 'Đang cập nhật...'}</p>
                      <div className="text-stone-500 text-sm line-clamp-2 font-serif" dangerouslySetInnerHTML={{ __html: newsItems[0]?.desc || 'Kính mời quý cộng đoàn theo dõi...' }} />
                    </div>
                    <div className="mt-8 pt-6 border-t border-pink-50 text-center"><Phone className="mx-auto text-pink-600 mb-1" size={16} /><div className="text-lg font-bold text-pink-950">028.123.456.78</div><span className="text-[9px] text-stone-400 uppercase font-bold tracking-widest">Văn phòng giáo xứ</span></div>
                  </div>
              </div>
            </section>

            <section id="news-section" className="py-16 md:py-20 bg-white">
              <div className="max-w-6xl mx-auto px-4 text-stone-900">
                <div className="text-center mb-10 relative">
                  <h2 className="text-pink-600 font-bold uppercase tracking-widest text-[10px] mb-2">Truyền thông Công giáo</h2>
                  <h3 className="text-3xl md:text-4xl font-serif text-pink-950 font-bold tracking-tight">Bản Tin Đền Thánh</h3>
                  <div className="h-[2px] w-16 bg-pink-300 mx-auto mt-4"></div>
                  {isAdmin && <button onClick={() => { setTempNews({ id: Date.now(), title: '', date: getTodayFormatted(), category: 'Tin Tức', desc: '', image: '', content: '', isFeatured: false }); setEditingNews('new'); }} className="absolute right-0 top-0 px-4 py-2 bg-pink-700 text-white text-[11px] font-bold rounded shadow-md transition-all active:scale-95">+ Thêm Bản Tin</button>}
                </div>
                
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:h-[400px]">
                  {newsItems.length > 0 && (
                    <div 
                      className="lg:col-span-7 group cursor-pointer relative flex flex-col rounded-xl overflow-hidden shadow-md h-[300px] lg:h-full"
                      onClick={() => { setLastTab('Home'); setSelectedNews(newsItems[0]); setActiveTab('NewsDetail'); window.scrollTo(0,0); }}
                    >
                      {isAdmin && <button onClick={(e) => { e.stopPropagation(); setTempNews(newsItems[0]); setEditingNews(newsItems[0].id); }} className="absolute top-3 right-3 z-20 p-1.5 bg-pink-600 text-white rounded-full shadow-md transition active:scale-90"><Edit3 size={14} /></button>}
                      <img src={newsItems[0].image} alt={newsItems[0].title} className="absolute inset-0 w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105 bg-stone-100" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent"></div>
                      <div className="absolute bottom-0 left-0 w-full p-6 z-10 flex flex-col justify-end">
                        <div><span className="bg-pink-600 text-white text-[9px] font-bold px-2 py-1 rounded inline-block mb-3 shadow-sm uppercase tracking-wider">{newsItems[0].category}</span></div>
                        <h4 className="font-serif font-bold text-xl md:text-2xl text-white mb-2 group-hover:text-pink-300 transition leading-tight line-clamp-2">{newsItems[0].title}</h4>
                        <div className="text-stone-300 text-sm line-clamp-2 mb-3 leading-relaxed font-serif [&_p]:mb-0" dangerouslySetInnerHTML={{ __html: newsItems[0].desc }} />
                        <p className="text-[10px] font-bold text-pink-300 uppercase flex items-center tracking-widest"><Calendar size={10} className="mr-1.5" />{newsItems[0].date}</p>
                      </div>
                    </div>
                  )}

                  <div className="lg:col-span-5 flex flex-col gap-3 h-full overflow-y-auto custom-scrollbar pr-1">
                    {newsItems.slice(1, 6).map((item) => (
                      <div 
                        key={item.id} 
                        className="group cursor-pointer relative flex items-center gap-3 bg-white p-2.5 rounded-lg hover:bg-pink-50 transition border border-pink-100 hover:border-pink-200 shadow-sm"
                        onClick={() => { setLastTab('Home'); setSelectedNews(item); setActiveTab('NewsDetail'); window.scrollTo(0,0); }}
                      >
                        {isAdmin && <button onClick={(e) => { e.stopPropagation(); setTempNews(item); setEditingNews(item.id); }} className="absolute top-1.5 right-1.5 z-20 p-1 bg-pink-600 text-white rounded-full shadow-sm transition active:scale-90"><Edit3 size={10} /></button>}
                        <div className="w-24 h-20 md:w-28 md:h-24 flex-shrink-0 relative overflow-hidden rounded-md shadow-sm">
                          <img src={item.image} alt={item.title} className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 bg-stone-100" />
                        </div>
                        <div className="flex-1 flex flex-col py-1 pr-2">
                          <span className="text-[9px] font-bold text-pink-600 uppercase tracking-widest mb-1">{item.category}</span>
                          <h4 className="font-serif font-bold text-sm text-pink-950 mb-1.5 group-hover:text-pink-700 transition leading-snug line-clamp-2">{item.title}</h4>
                          <p className="text-[9px] font-bold text-stone-500 uppercase tracking-widest flex items-center mt-auto"><Calendar size={10} className="mr-1" />{item.date}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="text-center mt-8">
                  <button onClick={() => { setActiveTab('News'); window.scrollTo(0,0); }} className="px-8 py-3 border border-pink-600 text-pink-600 font-bold text-[11px] uppercase tracking-widest rounded shadow-sm hover:bg-pink-50 transition">Xem Tất Cả Tin Tức</button>
                </div>
              </div>
            </section>
          </>
  ...
  return (
    <div className="min-h-screen bg-white font-serif selection:bg-pink-100 antialiased relative overflow-x-hidden">
      <style>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { rgba(219, 39, 119, 0.6); }
      `}</style>
      ...
      <header className={`fixed w-full z-[100] transition-all duration-500 ${isSolidHeader ? 'bg-white shadow-md py-3 border-b border-pink-50' : 'bg-transparent py-5 text-white'}`}>
        <div className="max-w-7xl mx-auto px-4 lg:px-6 flex justify-between items-center">
          <div className="flex items-center space-x-3 cursor-pointer group" onClick={() => setActiveTab('Home')}>
            <Logo sizeClass="w-10 h-10 md:w-12 md:h-12" isSolid={isSolidHeader} />
            <div className={`border-l pl-4 transition-colors hidden sm:block ${isSolidHeader ? 'border-pink-200' : 'border-white/20'}`}>
              ...
              <p className={`text-[9px] font-bold uppercase tracking-[0.2em] mt-1 ${isSolidHeader ? 'text-pink-700' : 'text-pink-300'}`}>Đền Thánh Tử Đạo Việt Nam</p>
            </div>
          </div>
          
          <nav className="hidden lg:flex items-center space-x-5 xl:space-x-6">
            {navLinks.map((link) => (
              <button key={link.id} onClick={() => setActiveTab(link.id)} className={`text-[11px] font-bold uppercase tracking-widest relative pb-1.5 group transition-colors ${activeTab === link.id ? 'text-pink-600' : (isSolidHeader ? 'text-stone-600 hover:text-pink-900' : 'text-white/80 hover:text-white')}`}>
            ...
            <button className={`p-1.5 transition-all hover:scale-110 ${isSolidHeader ? 'text-stone-400 hover:text-pink-600' : 'text-white/50 hover:text-white'}`}><Search size={18} /></button>
          </nav>
        ...
        <div className={`fixed inset-0 top-0 left-0 w-full h-screen bg-pink-950/98 backdrop-blur-md z-[90] transition-all duration-500 lg:hidden flex flex-col items-center justify-center p-8 ${isMenuOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
           <div className="w-full flex flex-col items-center space-y-8 text-white">
              <Logo sizeClass="w-20 h-20" isSolid={false} />
              <div className="flex flex-col items-center space-y-5 w-full">
                {navLinks.map((link) => (<button key={link.id} onClick={() => { setActiveTab(link.id); setIsMenuOpen(false); }} className={`text-lg font-bold uppercase tracking-widest transition-all ${activeTab === link.id ? 'text-pink-400 scale-105' : 'text-white/70 hover:text-white'}`}>{link.name}</button>))}
              </div>
              <div className="h-px w-16 bg-pink-500/40"></div>
              <button onClick={() => setIsMenuOpen(false)} className="text-white/50 uppercase tracking-widest text-[10px] font-bold border border-white/20 px-5 py-2 rounded-full hover:bg-white/10 transition-all">Đóng Menu</button>
           </div>
        </div>
      </header>
      ...
      <footer className="bg-[#1a0f12] text-pink-200/60 pt-20 pb-10 relative border-t-4 border-pink-950 overflow-hidden">
        <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 relative z-10 text-left">
          <div className="col-span-1 lg:col-span-1 flex flex-col">
            <div className="flex items-center space-x-3 mb-6"><Logo sizeClass="w-12 h-12" isSolid={true} /><h2 className="font-bold text-white text-lg uppercase tracking-widest">Hoàng Yên</h2></div>
            <p className="text-sm italic text-pink-100 opacity-60 leading-relaxed mb-6">"Lạy Chúa, xin cho chúng con được hiệp nhất trong tình yêu và sự phục vụ, theo gương các bậc tiền nhân đã đổ máu đào minh chứng đức tin."</p>
          </div>
          <div>
            <h3 className="text-pink-100 font-bold text-xs uppercase mb-6 border-b border-pink-900/50 pb-3 tracking-widest">Thông Tin Liên Hệ</h3>
            <ul className="space-y-3 text-sm opacity-80 font-serif leading-relaxed">
              <li className="flex items-start gap-2"><MapPin className="text-pink-500 mt-0.5 flex-shrink-0" size={16} /> <span>123 Các Thánh Tử Đạo, Phường Hoàng Yên, Quận 1, TP. HCM</span></li>
              <li className="flex items-center gap-2"><Phone className="text-pink-500 flex-shrink-0" size={16} /> <span>(028) 1234 5678</span></li>
              <li className="flex items-center gap-2"><Mail className="text-pink-500 flex-shrink-0" size={16} /> <span>vanphong@giaoxu.vn</span></li>
            </ul>
          </div>
          <div>
            <h3 className="text-pink-100 font-bold text-xs uppercase mb-6 border-b border-pink-900/50 pb-3 tracking-widest">Liên Kết Nhanh</h3>
            <ul className="space-y-3 text-[11px] uppercase opacity-80 tracking-widest">
              <li className="hover:text-pink-400 cursor-pointer transition" onClick={() => setActiveTab('Liturgy')}>Lịch Phụng Vụ</li>
              <li className="hover:text-pink-400 cursor-pointer transition" onClick={() => setActiveTab('Catechism')}>Giáo lý</li>
              <li className="hover:text-pink-400 cursor-pointer transition" onClick={() => setActiveTab('News')}>Tin Tức Giáo Phận</li>
              <li className="hover:text-pink-400 cursor-pointer transition" onClick={() => setActiveTab('Liturgy')}>Thủ tục Bí tích</li>
            </ul>
          </div>
          <div>
            <h3 className="text-pink-100 font-bold text-xs uppercase mb-6 border-b border-pink-900/50 pb-3 tracking-widest">Kết Nối Cộng Đoàn</h3>
            <ul className="space-y-3 text-sm opacity-90 font-serif">
              <li>
                <a href="#" className="flex items-center gap-3 hover:text-pink-300 transition-colors">
                  <div className="w-8 h-8 rounded-full bg-pink-900/40 flex items-center justify-center border border-pink-800/50"><FacebookIcon size={14} className="text-pink-200" /></div>
                  <span>Fanpage Giáo Xứ</span>
                </a>
              </li>
            </ul>
          </div>
        </div>
        <div className="max-w-6xl mx-auto px-4 mt-16 pt-6 border-t border-white/10 text-[9px] uppercase font-bold opacity-40 text-center flex flex-col md:flex-row justify-between items-center gap-3 tracking-widest">
          <p>© 2023 GIÁO XỨ HOÀNG YÊN.</p>
          <span onClick={() => isAdmin ? setIsAdmin(false) : setShowLoginModal(true)} className="cursor-pointer hover:text-white transition-all hover:scale-105 active:scale-95">{isAdmin ? 'Thoát Quản Trị' : 'Admin'}</span>
        </div>
      </footer>
      ...
      {showLoginModal && (
        <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white p-8 rounded-xl shadow-2xl max-w-sm w-full text-center border-t-4 border-pink-600 relative">
            ...
            <input type="password" placeholder="Nhập mật khẩu" className={`w-full border p-3 mb-2 text-center text-sm outline-none focus:border-pink-500 font-serif rounded ${loginError ? 'border-red-400' : 'border-pink-200'}`} value={password} onChange={(e) => {setPassword(e.target.value); setLoginError('');}} onKeyDown={(e) => e.key === 'Enter' && handleLogin()} />
            ...
            <div className="flex gap-3"><button className="flex-1 bg-stone-100 py-3 font-bold uppercase text-[10px] tracking-widest rounded hover:bg-stone-200 transition" onClick={() => {setShowLoginModal(false); setLoginError(''); setPassword('');}}>Hủy</button><button className="flex-1 bg-pink-700 text-white py-3 font-bold uppercase text-[10px] shadow-md rounded active:scale-95 transition-all hover:bg-pink-800" onClick={handleLogin}>Xác Nhận</button></div>
          </div>
        </div>
      )}
      ...
      {editingLiturgyEvent && (
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-8 rounded-xl shadow-2xl max-w-md w-full border-t-4 border-pink-600 relative">
            <h3 className="text-xl font-bold text-pink-950 mb-6 uppercase text-center tracking-tight">Sự Kiện Ngày {tempLiturgyEvent.date.split('-').reverse().join('/')}</h3>
            ...
            <select className="w-full border border-pink-200 p-3 rounded mb-4 text-sm font-bold bg-white outline-none cursor-pointer focus:border-pink-500" value={tempLiturgyEvent.colorType} onChange={(e) => setTempLiturgyEvent({...tempLiturgyEvent, colorType: e.target.value})}>
              {Object.keys(litColors).map(key => (<option key={key} value={key}>{litColors[key].name}</option>))}
            </select>
            ...
            <div className="flex gap-3 pt-6 border-t mt-4">
              <button className="text-red-600 px-4 py-3 font-bold text-[10px] uppercase border border-red-100 hover:bg-red-50 transition-all rounded" onClick={() => { setLiturgyEvents(liturgyEvents.filter(e => e.date !== tempLiturgyEvent.date)); setEditingLiturgyEvent(false); }}>Xóa</button>
              <div className="flex-1"></div>
              <button className="bg-stone-100 px-6 py-3 rounded font-bold text-[10px] uppercase tracking-widest" onClick={() => setEditingLiturgyEvent(false)}>Hủy</button>
              <button className="bg-pink-700 text-white px-8 py-3 rounded font-bold text-[10px] uppercase tracking-widest shadow-md active:scale-95 transition-all" onClick={() => {
                if (tempLiturgyEvent.title.trim() === '') return alert('Vui lòng nhập tên sự kiện');
                const newEvents = liturgyEvents.filter(e => e.date !== tempLiturgyEvent.date);
                newEvents.push(tempLiturgyEvent);
                setLiturgyEvents(newEvents);
      ...
      {editingQuote && (
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border-t-4 border-pink-600 custom-scrollbar">
            ...
            <div className="flex gap-3"><button className="flex-1 bg-stone-100 py-3 rounded font-bold uppercase text-[10px] tracking-widest" onClick={() => setEditingQuote(false)}>Hủy</button><button className="flex-[2] bg-pink-700 text-white rounded py-3 font-bold uppercase text-[10px] shadow active:scale-95 transition-all" onClick={() => { setQuote(tempQuote); setEditingQuote(false); }}>Lưu Lại</button></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border-t-4 border-pink-600 custom-scrollbar relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
              ...
              <div><label className="text-[10px] font-bold uppercase tracking-widest text-stone-400 mb-1 block">Chuyên mục</label><select className="w-full border border-pink-200 p-3 rounded text-sm bg-white outline-none cursor-pointer focus:border-pink-500" value={tempNews.category} onChange={(e) => setTempNews({...tempNews, category: e.target.value})}><option value="Tin Tức">Tin Tức</option><option value="Sự kiện">Sự kiện</option><option value="Giáo lý">Giáo lý</option><option value="Thông báo">Thông báo</option></select></div>
            </div>
            ...
            <div className="flex gap-4 border-t pt-5 mt-6">{editingNews !== 'new' && <button className="text-red-600 px-6 py-3 font-bold text-[10px] uppercase border border-red-100 hover:bg-red-50 transition-all rounded" onClick={() => { setNewsItems(newsItems.filter(n => n.id !== tempNews.id)); if (selectedNews?.id === tempNews.id) { setSelectedNews(null); setActiveTab(lastTab); } setEditingNews(null); }}>Xóa Bản Tin</button>}<div className="flex-1"></div><button className="bg-stone-100 px-8 py-3 rounded font-bold text-[10px] uppercase tracking-widest" onClick={() => setEditingNews(null)}>Hủy Bỏ</button><button className="bg-pink-700 text-white px-10 py-3 rounded font-bold text-[10px] uppercase tracking-widest shadow-md active:scale-95 transition-all hover:bg-pink-800" onClick={() => { if (editingNews === 'new') setNewsItems([tempNews, ...newsItems]); else { setNewsItems(newsItems.map(n => n.id === tempNews.id ? tempNews : n)); if (selectedNews?.id === tempNews.id) setSelectedNews(tempNews); } setEditingNews(null); }}>Lưu Bài Viết</button></div>
          </div>
        </div>
      )}
      ...
      {editingHistory && (
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border-t-4 border-pink-600 custom-scrollbar relative">
            ...
            <div className="flex gap-4 pt-6 border-t mt-6"><div className="flex-1"></div><button className="bg-stone-100 px-8 py-3 rounded font-bold text-[10px] uppercase tracking-widest" onClick={() => setEditingHistory(false)}>Hủy</button><button className="bg-pink-700 text-white px-10 py-3 rounded font-bold text-[10px] uppercase tracking-widest shadow-md active:scale-95 transition-all" onClick={() => { setHistoryData(tempHistory); setEditingHistory(false); }}>Cập Nhật Lịch Sử</button></div>
          </div>
        </div>
      )}
      ...
      {editingHeritageTitle && (
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-8 rounded-xl shadow-2xl max-w-sm w-full border-t-4 border-pink-600 relative">
            ...
            <div className="flex gap-3"><button className="flex-1 bg-stone-100 py-3 rounded font-bold uppercase text-[10px]" onClick={() => setEditingHeritageTitle(false)}>Hủy</button><button className="flex-1 bg-pink-700 text-white py-3 rounded font-bold uppercase text-[10px] shadow" onClick={() => { setHeritageTitle(tempHeritageTitle); setEditingHeritageTitle(false); }}>Lưu Lại</button></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-xl w-full max-h-[90vh] overflow-y-auto border-t-4 border-pink-600 custom-scrollbar relative">
            ...
            <div className="flex gap-4 pt-6 border-t mt-6">
              {editingHeritageItem !== 'new' && <button className="text-red-600 px-6 py-3 font-bold text-[10px] uppercase border border-red-100 hover:bg-red-50 transition-all rounded" onClick={() => { setHeritageList(heritageList.filter(item => item.id !== tempHeritageItem.id)); setEditingHeritageItem(null); }}>Xóa Vị Thánh</button>}
              <div className="flex-1"></div>
              <button className="bg-stone-100 px-6 py-3 rounded font-bold text-[10px] uppercase tracking-widest" onClick={() => setEditingHeritageItem(null)}>Hủy</button>
              <button className="bg-pink-700 text-white px-8 py-3 rounded font-bold text-[10px] uppercase tracking-widest shadow-md active:scale-95 transition-all" onClick={() => { if (editingHeritageItem === 'new') setHeritageList([tempHeritageItem, ...heritageList]); else setHeritageList(heritageList.map(item => item.id === tempHeritageItem.id ? tempHeritageItem : item)); setEditingHeritageItem(null); }}>Lưu Lại</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingPastoral && (
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto border-t-4 border-pink-600 custom-scrollbar relative">
            ...
            <div className="flex gap-4 pt-6 border-t mt-6"><div className="flex-1"></div><button className="bg-stone-100 px-8 py-3 rounded font-bold text-[10px] uppercase tracking-widest" onClick={() => setEditingPastoral(false)}>Hủy</button><button className="bg-pink-700 text-white px-10 py-3 rounded font-bold text-[10px] uppercase tracking-widest shadow-md active:scale-95 transition-all" onClick={() => { setPastoralData(tempPastoral); setEditingPastoral(false); }}>Lưu Định Hướng</button></div>
          </div>
        </div>
      )}
      ...
      {editingMass && (
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-8 rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border-t-4 border-pink-600 custom-scrollbar">
            <h3 className="text-xl font-bold text-pink-950 mb-8 uppercase text-center tracking-tight">Sửa Lịch Lễ</h3>
            <div className="space-y-6">
              {tempMass.map((item, idx) => (
                <div key={idx} className="p-4 border border-pink-100 bg-pink-50/30 rounded-lg">
                  <label className="block text-[10px] font-bold text-pink-800 uppercase mb-2 tracking-widest">{item.day}</label>
                  <input className="w-full border border-pink-200 p-3 text-sm focus:border-pink-600 outline-none transition-all rounded font-bold text-pink-950" value={item.times.join(', ')} onChange={(e) => { const newMass = [...tempMass]; newMass[idx].times = e.target.value.split(',').map(t => t.trim()); setTempMass(newMass); }} placeholder="VD: 05:00, 17:30..." />
                </div>
              ))}
            </div>
            <div className="flex gap-4 pt-8 border-t mt-8"><button className="bg-stone-100 px-8 py-3 rounded font-bold text-[10px] uppercase tracking-widest" onClick={() => setEditingMass(false)}>Hủy</button><button className="flex-1 bg-pink-700 text-white py-3 rounded font-bold text-[10px] uppercase tracking-widest shadow-md active:scale-95 transition-all" onClick={() => { setMassSchedules(tempMass); setEditingMass(false); }}>Lưu Giờ Lễ Mới</button></div>
          </div>
        </div>
      )}
      ...
      {editingConfession && (
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-8 rounded-xl shadow-2xl max-w-sm w-full border-t-4 border-pink-600 relative">
            ...
            <div className="flex gap-3"><button className="flex-1 bg-stone-100 py-3 rounded font-bold uppercase text-[10px] tracking-widest" onClick={() => setEditingConfession(false)}>Hủy</button><button className="flex-1 bg-pink-700 text-white py-3 rounded font-bold uppercase text-[10px] tracking-widest shadow-md active:scale-95 transition-all" onClick={() => { setConfessionData(tempConfession); setEditingConfession(false); }}>Lưu Lại</button></div>
          </div>
        </div>
      )}
      ...
      {editingAdoration && (
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-8 rounded-xl shadow-2xl max-w-sm w-full border-t-4 border-pink-600 relative">
            ...
            <div className="flex gap-3"><button className="flex-1 bg-stone-100 py-3 rounded font-bold uppercase text-[10px] tracking-widest" onClick={() => setEditingAdoration(false)}>Hủy</button><button className="flex-1 bg-pink-700 text-white py-3 rounded font-bold uppercase text-[10px] tracking-widest shadow-md active:scale-95 transition-all" onClick={() => { setAdorationData(tempAdoration); setEditingAdoration(false); }}>Lưu Lại</button></div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border-t-4 border-pink-600 custom-scrollbar relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
              ...
              <div>
                ...
                <select className="w-full border border-pink-200 p-3 rounded text-sm font-bold bg-white outline-none cursor-pointer focus:border-pink-500" value={tempPilgrimage.status} onChange={(e) => setTempPilgrimage({...tempPilgrimage, status: e.target.value})}>
            ...
            <div className="flex gap-4 border-t pt-5 mt-6">
              {editingPilgrimage !== 'new' && <button className="text-red-600 px-6 py-3 font-bold text-[10px] uppercase border border-red-100 hover:bg-red-50 transition-all rounded" onClick={() => { setPilgrimagePlans(pilgrimagePlans.filter(p => p.id !== tempPilgrimage.id)); if (selectedPilgrimage?.id === tempPilgrimage.id) { setSelectedPilgrimage(null); setActiveTab('Pilgrimage'); } setEditingPilgrimage(null); }}>Xóa Kế Hoạch</button>}
              <div className="flex-1"></div>
              <button className="bg-stone-100 px-8 py-3 rounded font-bold text-[10px] uppercase tracking-widest" onClick={() => setEditingPilgrimage(null)}>Hủy Bỏ</button>
              <button className="bg-pink-700 text-white px-10 py-3 rounded font-bold text-[10px] uppercase tracking-widest shadow-md active:scale-95 transition-all hover:bg-pink-800" onClick={() => { 
                if (tempPilgrimage.title.trim() === '') return alert('Vui lòng nhập tên chương trình');
                if (editingPilgrimage === 'new') setPilgrimagePlans([tempPilgrimage, ...pilgrimagePlans]); 
                else { 
                  setPilgrimagePlans(pilgrimagePlans.map(p => p.id === tempPilgrimage.id ? tempPilgrimage : p)); 
                  if (selectedPilgrimage?.id === tempPilgrimage.id) setSelectedPilgrimage(tempPilgrimage); 
                } 
      ...
      {editingReception && (
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border-t-4 border-pink-600 custom-scrollbar relative">
            ...
            <div className="flex gap-4 pt-6 border-t mt-6">
              <button className="flex-1 bg-stone-100 py-3 rounded font-bold uppercase text-[10px] tracking-widest" onClick={() => setEditingReception(false)}>Hủy Bỏ</button>
              <button className="flex-[2] bg-pink-700 text-white py-3 rounded font-bold uppercase text-[10px] tracking-widest shadow-md active:scale-95 transition-all hover:bg-pink-800" onClick={() => { setReceptionInfo(tempReception); setEditingReception(false); }}>Lưu Lại</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingStats && (
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4">
          <div className="bg-white p-8 rounded-xl shadow-2xl max-w-sm w-full border-t-4 border-pink-600">
            ...
            <div className="flex gap-3"><button className="flex-1 bg-stone-100 py-3 rounded font-bold uppercase text-[10px] tracking-widest" onClick={() => setEditingStats(false)}>Hủy</button><button className="flex-1 bg-pink-700 text-white py-3 rounded font-bold uppercase text-[10px] tracking-widest shadow-md active:scale-95 transition-all" onClick={() => { setParishStats(tempStats); setEditingStats(false); }}>Lưu Lại</button></div>
          </div>
        </div>
      )}
    </div>

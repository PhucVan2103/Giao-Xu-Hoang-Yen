...
// ==========================================
// CẤU HÌNH FIREBASE (TẠM THỜI VÔ HIỆU HÓA)
// ==========================================
// Đã tạm thời comment lại để tránh lỗi "Missing or insufficient permissions" trong môi trường Sandbox
/*
...
const safeAppId = 'giao-xu-hoang-yen-demo-db';

// ==========================================
// COMPONENT DÙNG CHUNG & HELPER
// ==========================================
...
const initialNewsData = [
  ...
  { 
    ...
    image: 'https://images.unsplash.com/photo-1510936111840-65e151ad71bb?q=80&w=800&auto=format&fit=crop', imgFit: 'cover', imgScale: 1, imgPosX: 50, imgPosY: 50,
  ...
  { 
    ...
    image: 'https://images.unsplash.com/photo-1544627056-a4c330f3050c?q=80&w=800&auto=format&fit=crop', imgFit: 'cover', imgScale: 1, imgPosX: 50, imgPosY: 50,
  ...
  { 
    id: 6, title: 'Lịch tĩnh tâm Mùa Vọng 2023', date: '20/10/2023', category: 'Sự kiện', isFeatured: false,
    desc: '<p>Chương trình tĩnh tâm Mùa Vọng dành cho các giới trong giáo xứ sẽ bắt đầu từ...</p>',
    image: 'https://images.unsplash.com/photo-1438232992991-995b7058bbb3?q=80&w=800&auto=format&fit=crop', imgFit: 'cover', imgScale: 1, imgPosX: 50, imgPosY: 50,
    content: '<p>Giáo xứ thông báo lịch tĩnh tâm Mùa Vọng năm 2023. Kính mời cộng đoàn sắp xếp thời gian.</p>'
  }
];
...
export default function App() {
  ...
  // ==========================================
  // FIREBASE INITIALIZATION & SYNC (VÔ HIỆU HÓA)
  // ==========================================
  // Tạm thời vô hiệu hóa useEffect gọi Firebase
  /*
  useEffect(() => {
    if (!auth) return;
  ...
  */
  ...
  const saveConfigToDB = async (key, value) => {
    // Tạm thời lưu vào local state thay vì gọi Firebase
    switch(key) {
    ...
    try { await setDoc(doc(db, 'artifacts', safeAppId, 'public', 'data', 'config', 'main'), { [key]: value }, { merge: true }); } 
        ...
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border-t-4 border-pink-600 custom-scrollbar relative">
            ...
            <div className="flex gap-4 border-t pt-5 mt-6">
              {editingNews !== 'new' && <button className="text-red-600 px-6 py-3 font-bold text-[10px] uppercase tracking-widest border border-red-100 hover:bg-red-50 transition-all rounded" onClick={async () => { setNewsItems(newsItems.filter(n => n.id !== tempNews.id)); if (selectedNews?.id === tempNews.id) { setSelectedNews(null); setActiveTab(lastTab); } setEditingNews(null); }}>Xóa Bản Tin</button>}
              ...
              <button className="bg-pink-700 text-white px-10 py-3 rounded font-bold text-[10px] uppercase tracking-widest shadow-md active:scale-95 transition-all hover:bg-pink-800" onClick={async () => { 
                ...
                else { setNewsItems(newsItems.map(n => n.id === tempNews.id ? d : n)); if (selectedNews?.id === tempNews.id) setSelectedNews(d); }
                setEditingNews(null); 
              }}>Lưu Bài Viết</button>
            </div>
          </div>
      ...
      {editingPilgrimage !== null && tempPilgrimage && (
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border-t-4 border-pink-600 custom-scrollbar relative">
            ...
            <div className="flex gap-4 border-t pt-5 mt-6">
              {editingPilgrimage !== 'new' && <button className="text-red-600 px-6 py-3 font-bold text-[10px] uppercase tracking-widest border border-red-100 hover:bg-red-50 transition-all rounded" onClick={async () => { setPilgrimagePlans(pilgrimagePlans.filter(p => p.id !== tempPilgrimage.id)); if (selectedPilgrimage?.id === tempPilgrimage.id) { setSelectedPilgrimage(null); setActiveTab('Pilgrimage'); } setEditingPilgrimage(null); }}>Xóa Kế Hoạch</button>}
              <div className="flex-1"></div>
              ...
              <button className="bg-pink-700 text-white px-10 py-3 rounded font-bold text-[10px] uppercase tracking-widest shadow-md active:scale-95 transition-all hover:bg-pink-800" onClick={async () => { 
                ...
                else { setPilgrimagePlans(pilgrimagePlans.map(p => p.id === tempPilgrimage.id ? d : p)); if (selectedPilgrimage?.id === tempPilgrimage.id) setSelectedPilgrimage(d); }
        ...
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-6 rounded-xl shadow-2xl max-w-md w-full border-t-4 border-pink-600 relative">
            ...
            <div className="flex gap-3 pt-6 border-t mt-4">
              <button className="text-red-600 px-4 py-3 font-bold text-[10px] uppercase border border-red-100 hover:bg-red-50 transition-all rounded tracking-widest" onClick={async () => { setLiturgyEvents(liturgyEvents.filter(e => e.date !== tempLiturgyEvent.date)); setEditingLiturgyEvent(false); }}>Xóa</button>
              <div className="flex-1"></div>
      ...
      {editingLogo && (
        <div className="fixed inset-0 z-[200] bg-black/60 flex items-center justify-center p-4 animate-in zoom-in duration-200">
          <div className="bg-white p-8 rounded-xl shadow-2xl max-w-xl w-full border-t-4 border-pink-600 relative">
            ...
            <ImageAdjuster data={tempLogoConfig} setData={setTempLogoConfig} aspectClass="w-40 h-40 rounded-full mx-auto" />

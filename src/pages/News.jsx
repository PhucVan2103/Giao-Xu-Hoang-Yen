          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {featuredNews.map(item => (
                <div key={item.id} onClick={() => { setSelectedNews(item); navigate('/tin-tuc/chi-tiet'); }} className="group cursor-pointer relative flex flex-col rounded-2xl overflow-hidden shadow-lg border border-pink-50 h-[380px] bg-white hover:shadow-xl hover:border-pink-200 transition-all duration-300">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentRegularNews.map((item) => (
              <div key={item.id} onClick={() => { setSelectedNews(item); navigate('/tin-tuc/chi-tiet'); }} className="group cursor-pointer relative bg-white p-4 rounded-xl border border-pink-50 shadow-sm hover:border-pink-300 hover:shadow-md transition-all flex flex-col">
...
export function NewsDetail({ isAdmin, selectedNews, setTempNews, setEditingNews }) {
  const navigate = useNavigate();
  if (!selectedNews) return <Navigate to="/tin-tuc" replace />;
